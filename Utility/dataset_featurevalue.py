...
def no_dataset_trainval_multi_fv(dlists, **dataset_params,):
    ...
    no_dataset = remove_outliers(list1_nmasi, z_thresh)
...
def no_dataset_test_multi_fv(dlists, **dataset_params):
...
def remove_outliers(train_data, z_thresh):
    ...
    for prelabel in range(43):
        ...
        A = (nolabel_train_data - np.mean(nolabel_train_data, axis=0))
        B = np.std(nolabel_train_data, axis=0)
        z_scores = np.abs(np.divide(A, B, out=np.zeros_like(A), where=B!=0))
